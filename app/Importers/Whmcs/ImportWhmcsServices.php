    ...
    public function run(callable $log): void
    {
        ...
        while (true) {
            ...
            foreach ($services as $record) {
                $sourceRecordId = $record['serviceid'];
                $rowHash        = hash('sha256', json_encode($record));
                $existing       = DB::table('source_whmcs_services')
                    ->where('source_system', $this->system)
                    ->where('source_record_id', $sourceRecordId)
                    ->first();

                if ($existing === null) {
                    DB::table('source_whmcs_services')->insert([
                    ...
                    DB::table('source_whmcs_services')
